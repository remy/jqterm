/**
 * Netlify-compatible WASM jq wrapper
 * 
 * This module provides an async wrapper for the jq WASM module
 * that works in Netlify Functions. Key differences from lib/jq-node.js:
 * - Async initialization to wait for WASM runtime
 * - Uses promise-based API
 * - Proper handling of Module.onRuntimeInitialized
 */
...
async function jq(jsonStr, query, options) {
  ...
  // Uncomment for debugging: console.log(mainOptions, FILE_DATA, jsonStr.substring(0, 50), query);
