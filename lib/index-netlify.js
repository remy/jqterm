/**
 * Netlify-compatible library exports
 * 
 * This module provides the same exports as lib/index.js but uses
 * the async jq wrapper (lib/jq-node-netlify.js) suitable for
 * Netlify Functions.
 */

const LRU = require('lru-cache');
const fs = require('fs');
const { jq } = require('./jq-node-netlify');

const { promisify } = require('util');
const VERSION = require('../package.json').version;
const readFile = promisify(fs.readFile);
const writeFile = promisify(fs.writeFile);
const tmpdir = require('os').tmpdir();

const options = {
  max: 500,
  maxAge: 1000 * 60 * 60,
};
const cache = new LRU(options);

const makeGistBody = ({ body, guid }) => ({
  description: guid,
  public: false,
  files: {
    'jace.json': {
      content: body.toString(),
    },
  },
});

const run = async ({ query, input, options = {}, id }) => {
  const args = [];
  if (options.slurp) args.push('-s');
  if (options.rawInput) args.push('-R');
  if (options.nullInput) args.push('-n');

  try {
    // Convert input to string if it's an object
    let inputStr = input;
    if (typeof input === 'object') {
      inputStr = JSON.stringify(input);
    }
    
    const result = await jq(inputStr, query, args);
    return result;
  } catch (e) {
    if (e.message && e.message.includes('Could not open file')) {
      return { status: 404, result: { error: e.message } };
    }

    console.error('JQ execution error:', e.message);

    const error = e.message ? e.message.replace(/^.*:\d+\):\s/, '').trim() : 'Unknown error';
    return { status: 500, result: { error: e.message || error } };
  }
};

const writeToFile = ({ id, files, description }) => {
  const filename = Object.keys(files).find((_) => _.endsWith('.json'));

  if (!files[filename]) {
    console.error('No .json file found in gist files');
    const e = new Error('could not create back end data');
    e.code = 500;
    throw e;
  }

  let payload = files[filename].content;

  // Write to /tmp for serverless environment
  const filepath = `${tmpdir}/${id}.json`;
  writeFile(filepath, payload, 'utf8').catch((error) => {
    if (error) console.error('Failed to write temp file:', error.message);
  });

  // body.description is the user guid
  cache.set(id, description);

  return { id, payload };
};

const getFilename = (id) => `${tmpdir}/${id}`;

module.exports = {
  makeGistBody,
  writeToFile,
  getFilename,
  cache,
  VERSION,
  readFile,
  run,
};
